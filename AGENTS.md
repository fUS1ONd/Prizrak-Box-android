...
`versionName` и `versionCode` — обычные константы в корневом `build.gradle.kts`
(сейчас `1.1` и `101`, достались от апстрима). Из них же собирается имя файла:
`archivesBaseName = "prizrak-box-$versionName"`, а флейвор добавляет суффикс
(`.Alpha`, `.Meta`).

Два режима сборки обходятся с ними по-разному:

- **Pre-release** берёт константы **как есть** и ничего не бампит. Поэтому все
  альфа-сборки остаются `1.1`, пока строку не поправят руками, а тег
  `Prerelease-alpha` каждый раз перезаписывается. Различать альфы по версии нельзя —
  только по дате сборки.
...
- Тег обязан иметь **ровно три компонента** (`v1.2.3`). `versionCode` считается как
  ...
  ошибки.
- `versionCode` должен только расти: Android ставит обновление поверх лишь при
  большем или равном коде. Сейчас в файле `101`, так что любой релиз от `v1.1.0`
  (`101000`) это условие выполняет с запасом.
